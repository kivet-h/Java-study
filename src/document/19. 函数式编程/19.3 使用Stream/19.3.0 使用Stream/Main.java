...
// 再次划重点：这个 Stream 和 List 也不一样，
// List 存储的每个元素都是已经存储在内存中的某个Java对象，
// 而 Stream 输出的元素可能并没有预先存储在内存中，而是实时计算出来的。
...
// 因为这个 streamNxN 也有无限多个元素，要打印它，必须首先把无限多个元素变成有限个元素，
// 可以用 limit() 方法截取前100个元素，最后用 forEach() 处理每个元素，
// 这样，我们就打印出了前100个自然数的平方：
// Stream<BigInteger> naturals = createNaturalStream();
// naturals.map(n -> n.multiply(n)) // 1, 4, 9, 16, 25...
//         .limit(100)
//         .forEach(System.out::println); // 1, 4, 9, 16, 25...   // 打印前100个自然数的平方值（0², 1², 2², ..., 99²）。
...
//         Stream<BigInteger> naturals = createNaturalStream();
//         naturals.map(n -> n.multiply(n)).limit(100).forEach(System.out::println);  // 1, 4, 9, 16, 25...   // 打印前100个自然数的平方值（0², 1², 2², ..., 99²）。
//     }
//
...
// Stream 的另一个特点是，一个 Stream 可以轻易地转换为另一个 Stream ，而不是修改原Stream 本身。
...
// 例如，创建一个全体自然数的 Stream ，不会进行计算，把它转换为上述 s2 这个 Stream ，也不会进行计算。
// 再把 s2 这个无限 Stream 转换为 s3 这个有限的 Stream ，也不会进行计算。
// 只有最后，调用 forEach 确实需要 Stream 输出的元素时，才进行计算。
