...
//        String second =  list.get(1);  // 为什么null可以赋值给String？ 答：String 是引用类型（类类型），它的变量存的是内存地址，null 可以赋给任何引用类型（类、接口、数组）（基本类型（int、boolean 等）才不能赋 null）
...
//有童鞋可能觉得使用Iterator访问List的代码比使用索引更复杂。但是，要记住，通过Iterator遍历List永远是最高效的方式。
...
//实际上，只要实现了Iterable接口的集合类都可以直接用for each循环来遍历，Java编译器本身并不知道如何遍历集合对象，但它会自动把for each循环变成Iterator的调用，
...
//    at java.base/java.util.ImmutableCollections$ListN.toArray(ImmutableCollections.java:750)
//    at Main.main(Main.java:336)
...
// 根据List接口的文档(https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/List.html#toArray%28T%5B%5D%29)，我们可以知道：


// 如果传入的数组不够大，那么List内部会创建一个新的刚好够大的数组，填充后返回；
// 如果传入的数组比List元素还要多，那么填充完元素后，剩下的数组元素一律填充null。

// 传入toArray的数组比List小时，自动创建一个刚好够大的数组，即3个长度
//import java.util.List;
//public class Main {
//    public static void main(String[] args) {
//        List<Integer> list = List.of(12, 34, 56);
...
// 传入toArray的数组比List大时，填充完元素后剩下的一律填充null
//import java.util.List;
//public class Main {
//    public static void main(String[] args) {
//        List<Integer> list = List.of(12, 34, 56);
//        Number[] array = list.toArray(new Number[4]);
//        for (Number n : array) {
//            System.out.println(n);
//        }
//    }
