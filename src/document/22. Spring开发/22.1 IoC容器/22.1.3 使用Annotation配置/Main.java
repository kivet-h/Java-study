/**
 * 22.1.3 使用Annotation配置 （示例代码：src/main/java/com/learnjava/Annotation）
 */
...
//使用@Autowired就相当于把指定类型的Bean注入到指定的字段中。和XML配置相比，@Autowired大幅简化了注入，
// 因为它不但可以写在set()方法上，还可以直接写在字段上，甚至可以写在构造方法中：
//@Component
//public class UserService {
//    MailService mailService;
//
...
//整个工程结构如下见：src/main/java/com/learnjava/Annotation
...
//使用@ComponentScan非常方便，但是，我们也要特别注意包的层次结构。
// 通常来说，启动配置AppConfig位于自定义的顶层包（例如com.learnjava.Annotation），其他Bean按类别放入子包。
