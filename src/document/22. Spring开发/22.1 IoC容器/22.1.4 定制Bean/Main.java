...
// 还有一种Bean，我们每次调用 getBean(Class) ，容器都返回一个新的实例，这种Bean称为Prototype（原型），它的生命周期显然和Singleton不同。
// 声明一个Prototype的Bean时，需要添加一个额外的 @Scope 注解：
// @Component
...
// ============================================================ 注入List ============================================================
// src.main
// └── java.com.dingZhiBean
//    ├── Validator.java          # 接口
//    ├── EmailValidator.java     # 实现类
//    ├── PasswordValidator.java  # 实现类
//    ├── NameValidator.java      # 实现类
//    ├── Validators.java         # 聚合类
//    └── AppConfig.java          # 配置类

// 示例代码见：src/main/java/com/dingZhiBean

// 注意到 Validators 被注入了一个 List<Validator> ，
// Spring会自动把所有类型为 Validator的Bean装配为一个 List 注入进来，这样一来，我们每新增一个 Validator 类型，就自动被Spring装配到 Validators 中了，非常方便。
...
// ============================================================ 可选注入 ============================================================
// 默认情况下，当我们标记了一个 @Autowired 后，Spring如果没有找到对应类型的Bean，它会抛出NoSuchBeanDefinitionException 异常。
...
// 我们通常会定义一个 init() 方法进行初始化，定义一个shutdown() 方法进行清理，然后，引入JSR-250定义的Annotation：
