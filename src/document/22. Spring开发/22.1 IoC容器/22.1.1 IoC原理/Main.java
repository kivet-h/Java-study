...
//为了从数据库查询书籍，BookService持有一个DataSource。
// 为了实例化一个HikariDataSource，又不得不实例化一个HikariConfig。
...
//因为UserService也需要访问数据库，
// 因此，我们不得不也实例化一个HikariDataSource。
...
//CartServlet创建了BookService，在创建BookService的过程中，又创建了DataSource组件。
//这种模式的缺点是，一个组件如果要使用另一个组件，必须先知道如何正确地创建它。



//在IoC模式下，控制权发生了反转，即从应用程序转移到了IoC容器，所有组件不再由应用程序自己创建和配置，而是由IoC容器负责，
